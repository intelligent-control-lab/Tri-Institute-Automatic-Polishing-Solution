...
namespace mr {
	...
	Eigen::MatrixXd Rot66(const Eigen::Matrix3d& R){
		Eigen::Matrix<double, 6, 6> R66 = Eigen::Matrix<double, 6, 6>::Zero();
		R66.topLeftCorner<3, 3>() = R;
		R66.bottomRightCorner<3, 3>() = R;
	 ...
	 * Returns: 6x6 Adjoint Representation of the matrix
	 */
	Eigen::MatrixXd Adjoint_PR(const Eigen::MatrixXd& T) {
		std::vector<Eigen::MatrixXd> R = TransToRp(T);
		Eigen::MatrixXd ad_ret(6, 6);
		ad_ret = Eigen::MatrixXd::Zero(6, 6);
		Eigen::MatrixXd zeroes = Eigen::MatrixXd::Zero(3, 3);
		ad_ret << R[0], VecToso3(R[1]) * R[0],
				zeroes, R[0];
		return ad_ret;
	}
